...
// Modified by SALAH — Fix pool rebirth1plus only + no wipe on empty scan
...
async function scanPool(poolKey) {
    ...
    // ✅ FIX : scan vide = on conserve le pool existant
    if (newPool.length === 0) {
        console.log('[SCAN] ' + config.label + ': scan vide, pool conservé (' + pools[poolKey].length + ' serveurs)');
        return;
    }

    // ✅ MERGE : on ajoute les nouveaux sans écraser les anciens
    const existingJobIds = new Set(pools[poolKey].map(s => s.jobId));
    const added = [];

    for (const server of newPool) {
        if (!existingJobIds.has(server.jobId)) {
            added.push(server);
        }
    }

    const merged = [...pools[poolKey], ...added];
    merged.sort((a, b) => b.score - a.score);
    pools[poolKey] = merged;
    ...
    const topScore = pools[poolKey].length > 0 ? pools[poolKey][0].score : 0;
    const bottomScore = pools[poolKey].length > 0 ? pools[poolKey][pools[poolKey].length - 1].score : 0;

    console.log('[SCAN] ' + config.label + ': +' + added.length + ' nouveaux | Pool total: ' + pools[poolKey].length + ' | Filtrés: ' + filteredOut + ' | Score top: ' + topScore + ' bottom: ' + bottomScore);
}
...
app.listen(PORT, () => {
    ...
    console.log('Fix: pool conservé si scan vide');
    console.log('================================================');
