// Godzilla Notifier Backend - OPTI MAX v6.2
// Cloudflare Worker proxy + Direct Roblox API fallback
// By SALAH
...
const CF_PROXY = 'https://roblox-proxy.salahelarabi03.workers.dev';
...
const stats = {
    ...
    reportsReceived: 0, reportsWithBrainrots: 0, logsReceived: 0, startedAt: Date.now()
};
...
async function fetchServers(cursor) {
    const url = CF_PROXY + '/v1/games/' + PLACE_ID
    ...
    try {
        ...
        console.error('[SCAN] Fetch failed: ' + e.message);
        return null;
    }
}

// ============================================================
// SCAN — fresh pool toutes les 5s
// ============================================================
...
async function scanPool() {
    ...
    if (pagesScanned === 0) {
        console.log('[SCAN] CF Worker unreachable, pool conserve (' + pool.length + ' serveurs)');
        return;
    }
    ...
    stats.totalScans++;
    console.log('[SCAN] ' + newPool.length + ' serveurs FRESH | Pages: ' + pagesScanned + ' | Filtres: ' + filteredOut + ' | Top: ' + (newPool[0] ? newPool[0].score : 0));
}
...
app.get('/', (req, res) => res.json({ name: 'Godzilla Notifier', version: '6.2', pool: pool.length, proxy: CF_PROXY }));
...
app.get('/jobs', (req, res) => {
    ...
    console.log('[JOBS] ' + username + ' -> ' + selected.jobId.substring(0, 12) + '... Score:' + selected.score + ' (' + (useTopScore ? 'TOP' : 'RND') + ') Reste: ' + pool.length);
...
app.get('/stats', (req, res) => {
    ...
    const m = uptime / 60;
    res.json({
        uptime, totalScans: stats.totalScans, jobsServed: stats.jobsServed,
        jobsServedTopScore: stats.jobsServedTopScore, jobsServedRandom: stats.jobsServedRandom,
        ...
        logsReceived: stats.logsReceived, activeBots: botHistory.size, activeJobs: jobLocks.size,
        recentBrainrots: recentBrainrots.length, pool: pool.length, quality: poolQualityStats
    });
});

app.get('/pool', (req, res) => res.json({ placeId: PLACE_ID, count: pool.length, quality: poolQualityStats, servers: pool.slice(0, 50) }));
