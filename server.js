...
// Dev by SALAH ⚡ | v3.2 - Brainrots TTL 60s + fetch natif
// ═══════════════════════════════════════════════════════════════
...
// ✅ NOUVEAU: fetch natif au lieu d'axios
...
app.get('/', (req, res) => {
    res.json({
        ...
        version: '3.2',
        endpoints: ['/health', '/jobs', '/report-data', '/stats', '/bots', '/dashboard', '/api/dashboard-data']
    });
});
...
'.stat-card { background: rgba(20, 30, 50, 0.6); border: 1px solid rgba(100, 150, 255, 0.2); border-radius: 12px; padding: 20px; text-align: center; }' +
...
'.brainrot-item { background: rgba(0, 60, 80, 0.3); border-left: 3px solid #00d4ff; border-radius: 6px; padding: 12px 15px; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center; position: relative; overflow: hidden; }' +
'.brainrot-info { flex: 1; }' +
'.brainrot-name { font-size: 14px; font-weight: 700; color: #00d4ff; }' +
'.brainrot-bot { font-size: 11px; color: #888; margin-top: 2px; }' +
'.brainrot-meta { display: flex; align-items: center; gap: 12px; }' +
'.brainrot-money { font-size: 16px; font-weight: 900; color: #00ff77; }' +
'.brainrot-timer { font-size: 13px; font-weight: 700; background: rgba(0, 0, 0, 0.4); padding: 4px 10px; border-radius: 12px; min-width: 60px; text-align: center; }' +
...
'.search-bar { width: 100%; background: rgba(10, 15, 25, 0.6); border: 1px solid rgba(100, 150, 255, 0.3); border-radius: 8px; padding: 10px 15px; color: #fff; margin-bottom: 15px; }' +
'.filter-tabs { display: flex; gap: 8px; margin-bottom: 15px; flex-wrap: wrap; }' +
'.filter-tab { background: rgba(10, 15, 25, 0.6); border: 1px solid rgba(100, 150, 255, 0.3); border-radius: 8px; padding: 8px 16px; color: #aaa; cursor: pointer; font-size: 13px; font-weight: 600; }' +
...
'.bot-card { background: rgba(15, 25, 45, 0.6); border-left: 4px solid #444; border-radius: 8px; padding: 14px; }' +
...
'.bot-status { font-size: 10px; font-weight: 700; padding: 3px 8px; border-radius: 4px; display: inline-block; margin-bottom: 8px; }' +
...
'.bot-name { font-weight: 700; font-size: 14px; margin-bottom: 8px; }' +
'.bot-stat { display: flex; justify-content: space-between; font-size: 12px; color: #aaa; padding: 2px 0; }' +
'.bot-stat .value { color: #fff; font-weight: 600; }' +
'.bot-brainrot { margin-top: 8px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.1); font-size: 12px; }' +
...
'<div class="stat-card pool"><div class="stat-label">POOL SERVEURS</div><div class="stat-value" id="stat-pool">0</div></div>' +
'<div class="stat-card scans"><div class="stat-label">TOTAL SCANS</div><div class="stat-value" id="stat-scans">0</div></div>' +
...
'<div class="section-title">Brainrots Trouves (>=40M) - restent 60s</div>' +
...
'    }' +
'    card.innerHTML = "<span class=\\"bot-status " + statusClass + "\\">" + statusText + "</span><div class=\\"bot-name\\">" + bot.botName + "</div><div class=\\"bot-stat\\"><span>Serveurs scannes:</span><span class=\\"value\\">" + bot.jobsReceived + "</span></div><div class=\\"bot-stat\\"><span>Dernier hop:</span><span class=\\"value\\">" + bot.lastSeenSeconds + "s</span></div><div class=\\"bot-stat\\"><span>JobID:</span><span class=\\"value\\">" + shortJobId(bot.currentJobId) + "</span></div>" + brainrotHtml;' +
...
app.listen(PORT, () => {
    console.log('===============================================');
    console.log('JobID Scanner v3.2 - Brainrots TTL 60s');
    console.log('===============================================');
