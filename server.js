// Godzilla Notifier Backend - v10.1 SIMPLE FRESH + RATE LIMIT
// Logique simple (plan Cloudflare PAYANT) :
//   - Scan toutes les ~10s, jusqu'a ~1500 serveurs (15 pages x 100)
//   - Blacklist des serveurs deja visites (pas de re-service)
//   - Chaque scan ECRASE la pool -> toujours du frais, zero accumulation
//
// 🔧 NOUVEAU v10.1 : RATE LIMIT par bot dans /jobs
//   -> Un bot ne peut demander que JOBS_MAX_PER_WINDOW serveurs par fenetre
//      de JOBS_RATE_WINDOW. Au-dela : reponse 429 (Rate limited).
//   -> Empeche un bot buggé (loop teleport-fail) de vider tout le pool.
//
// Base = v8.4 (dashboards, Discord, blacklist, bots intacts)
// By SALAH
...
const MAX_LOGS = 200;
const BLACKLIST_TTL = 5 * 60 * 1000;   // serveur visite = blackliste 5 min
const BOT_HISTORY_TTL = 6 * 60 * 60 * 1000;

// 🔧 RATE LIMIT par bot (anti-storm)
const JOBS_RATE_WINDOW = 10000;      // fenetre de 10 secondes
const JOBS_MAX_PER_WINDOW = 5;       // max 5 serveurs / 10s / bot (=> 30/min max)
...
// ═══════════════════════════════════════════════════════════════
// 🌐 PROXY = Cloudflare Worker (IPs differentes auto) + fallback
// ═══════════════════════════════════════════════════════════════
...
function cleanupBlacklist() {
    ...
    for (const [jobId, expiresAt] of blacklist.entries()) {
        if (expiresAt < now) blacklist.delete(jobId);
    }
}
...
// ═══════════════════════════════════════════════════════════════
// 🚀 SCAN : ~1500 serveurs, filtre 7/8, skip blacklist,
//    puis ECRASE la pool avec le frais.
// ═══════════════════════════════════════════════════════════════
async function scanPool() {
        ...
        for (const server of data.data) {
            ...
            if (server.playing === 7 || server.playing === 8) {
                ...
                if (!blacklist.has(server.id)) {
                    newPool.push({ jobId: server.id, players: server.playing, maxPlayers: server.maxPlayers });
                }
            }
        }
    ...
    if (pagesScanned === 0) {
        console.log('[SCAN] Worker + fallback dead, pool conservée (' + pool.length + ' serveurs)');
        return;
    }

    // ECRASE la pool avec le frais
    pool = newPool;

    stats.totalScans++;
    stats.lastScanPages = pagesScanned;
    stats.lastScanFound = newPool.length;
    stats.lastScanDuration = Date.now() - t0;

    console.log('[SCAN] Pages: ' + pagesScanned +
        ' | Scanned: ' + totalScanned +
        ' | 7/8: ' + total7or8 +
        ' | Pool (fresh): ' + newPool.length +
...
app.get('/', (req, res) => res.json({
    name: 'Godzilla Notifier', version: '10.1 Simple Fresh + Rate Limit',
    pool: pool.length,
    config: {
        scanInterval: SCAN_INTERVAL + 'ms',
        maxPages: MAX_PAGES,
        approxServersPerScan: MAX_PAGES * 100,
        poolMode: 'reset (fresh a chaque scan)',
        rateLimit: JOBS_MAX_PER_WINDOW + ' jobs / ' + (JOBS_RATE_WINDOW/1000) + 's / bot'
    }
}));
...
app.get('/jobs', (req, res) => {
    ...
    if (!botHistory.has(username)) {
        botHistory.set(username, { firstSeen: Date.now(), lastSeen: Date.now(), jobsReceived: 0, currentJobId: null, rateStart: Date.now(), rateCount: 0 });
    }
    const botData = botHistory.get(username);
    const now = Date.now();

    // ── 🔧 RATE LIMIT par bot ──────────────────────────────────────
    // Reset de la fenetre si elle est ecoulee
    if (!botData.rateStart || now - botData.rateStart > JOBS_RATE_WINDOW) {
        botData.rateStart = now;
        botData.rateCount = 0;
    }
    // Si le bot a deja consomme son quota -> 429
    if (botData.rateCount >= JOBS_MAX_PER_WINDOW) {
        botData.lastSeen = now;           // reste "vu" sur le monitor
        stats.jobsRateLimited++;
        const retryMs = JOBS_RATE_WINDOW - (now - botData.rateStart);
        console.log('[RATE] ' + username + ' bloque (' + botData.rateCount + '/' + JOBS_MAX_PER_WINDOW + ' sur ' + (JOBS_RATE_WINDOW/1000) + 's) -> retry dans ' + Math.ceil(retryMs/1000) + 's');
    ...
    // serveur visite -> blackliste (ne sera plus re-servi ni repris au prochain scan)
    blacklist.set(selected.jobId, Date.now() + BLACKLIST_TTL);
    botData.currentJobId = selected.jobId;
    stats.jobsServed++;
    console.log('[JOBS] ' + username + ' -> ' + selected.jobId.substring(0, 12) + '... Players: ' + selected.players + '/8 | Pool reste: ' + pool.length + ' | quota: ' + botData.rateCount + '/' + JOBS_MAX_PER_WINDOW);
...
app.get('/stats', (req, res) => {
    ...
    res.json({
        ...
        jobsRateLimited: stats.jobsRateLimited,
        ...
        rateLimit: { maxPerWindow: JOBS_MAX_PER_WINDOW, windowMs: JOBS_RATE_WINDOW, blocked: stats.jobsRateLimited },
